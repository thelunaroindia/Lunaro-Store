...
**Normal, full Cash on Delivery** may be available depending on how Fastr/Shopify Payments are configured — this must be manually verified in both dashboards before the site's COD copy asserts anything specific (see the launch checklist). **Partial COD (a percentage paid online, the remainder collected on delivery) was evaluated and is not implemented for this launch** — Fastr's integration as wired here has no native support for it (confirmed by inspecting its actual SDK surface, not assumed), and building it custom would require a Razorpay integration, `write_orders` Admin scope, and a payment-persistence layer this app does not have. It is not planned for this launch.
